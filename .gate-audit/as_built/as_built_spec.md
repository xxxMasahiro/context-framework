...
ディレクトリ責務、`./kit` サブコマンド仕様、各スクリプトの入出力、Evidence フォーマット、Self-check 設定とチェック一覧を定義する。
...
/home/masahiro/.gate-audit_root/.gate-audit/   <- KIT_ROOT
  ...
  tasks/                       <- トラッカー群 (6 ファイル)
    ...
    self_check_task_tracker.md       <- CI/QA タスク管理
  ...
  docs/                        <- 設計文書
    ...
    self-check/                <- CI/QA 設計 (4 ファイル)
      self_check_requirements.md
      self_check_spec.md
      self_check_implementation_plan.md
      self_check_plugin_guide.md
  config/                      <- 設定
    self-check.conf            <- CI/QA 設定 (config/self-check.conf:1-22)
  scripts/                     <- スクリプト群
    ...
    self-check.sh             <- CI/QA 実行器 (self-check.sh:1-401)
    ...
    lib/
      ...
      self_check_common.sh           <- CI/QA 共通ヘルパー
      gate_a.sh - gate_i.sh   <- 各 Gate 検証ロジック (9 ファイル)
      self_checks/             <- Self-check チェックプラグイン
  ...
  logs/
    ...
    self-check/
      baseline/
        last_run.json          <- Self-check 回帰検出用ベースライン
  handoff/
...
| set -euo pipefail | 全メインスクリプトで統一採用 | kit, verify_all.sh, verify_gate.sh, run_tests.sh, self-check.sh, generate_handoff.sh, evidence.sh |
...
| Medium-2 | Self-check プラグインの無検証 source | scripts/self-check.sh:250 | self_checks/ は git 管理下、外部配置シナリオなし（REQ-F10, SPEC-S09 準拠） |
...
| Low-6 | GATE_AUDIT_SEARCH_PATH 未サニタイズ find | 読み取り専用操作のみ |
| Low-7 | SC_NAMING_PATTERN ReDoS | config は git 管理下、外部攻撃ベクターなし |
...
| `scripts/self-check.sh` | 存在 | SSOT 仕様に記載なし（Self-check は後続追加） |
...
### SPEC-S05: ./kit self-check [CHECK...]

- **目的**: CI/QA チェックを実行する。
- **引数**: チェック名（tracker/evidence/ssot/docs/lint/naming/regression）を 0 個以上。省略時は全チェック。
- **処理**:
  1. `self-check.sh` に引数を渡す（kit:124）
  2. チェック後に `generate_handoff.sh` を実行（kit:129）
- **チェック決定優先度**: CLI 引数 > config `checks=` > all（self-check.sh:117-221）
- **config negation（否定構文）**: `checks=` フィールドで `!` プレフィックスによる除外指定をサポート（self-check.sh:146-193）。
  - **構文**: `checks=!lint,!naming` → lint と naming 以外の全チェックを実行
  - **排他ルール**: 正の指定（`tracker`）と負の指定（`!lint`）の混在はエラー（self-check.sh:159-162）
  - **未知チェック**: 否定モードで未知のキーが指定された場合は WARN を出力して無視（self-check.sh:178-179）
  - **解決フロー**:
    1. 全 config 項目を走査し `!` の有無を判定（self-check.sh:147-157）
    2. 否定モード: 全チェックから除外対象を除いたリストを返す（self-check.sh:164-193）
    3. 正モード: 指定されたチェックのみを返す（self-check.sh:194-216）
- **Exit code**: 0（全 PASS）/ 1（FAIL あり）
- **根拠**: kit:114-132, self-check.sh:1-401

### SPEC-S06: ./kit all

- **目的**: verify → test → self-check → handoff の一気通貫実行。
- **処理** (kit:134-186):
  1. Step 1/4: `verify_all.sh`
  2. Step 2/4: `run_tests.sh all`（存在しない場合は WARN でスキップ）
  3. Step 3/4: `self-check.sh all`（存在しない場合は WARN でスキップ）
...
- **処理** (kit:188-244):
  1. 6 トラッカーを走査（Verify, Test, As-built, Rebuild, PostRebld, Self-check）
...
### SPEC-S09: Self-check プラグイン自動発見

- **目的**: `scripts/lib/self_checks/cq_*.sh` を glob で取得し、メタデータからチェックを登録する。
...
- **ソートアルゴリズム（プラグインソート）**:
  1. 各プラグインの `@check_order` 値（既定 50）と `@check_key` を `order:key` ペアとして収集（self-check.sh:95-98）
  2. `sort -t: -k1,1n -k2,2` で数値昇順ソート → 同一 order 内はキー名アルファベット順で安定ソート（self-check.sh:104）
...
### SPEC-S14: scripts/lib/self_check_common.sh

- **目的**: Self-check 固有のヘルパー関数。
- **提供関数**:

| 関数 | 用途 | 根拠 |
|------|------|------|
| `sc_ts_jst()` / `sc_ts_utc()` / `sc_ts_label()` | タイムスタンプ | self_check_common.sh:11-13 |
| `sc_emit_header(check_name, check_id)` | Evidence ヘッダ出力 | self_check_common.sh:17-26 |
| `sc_emit_verdict(verdict, reason)` | 判定行出力 | self_check_common.sh:30-36 |
| `sc_count_checked(tracker_file)` | `[x]` カウント | self_check_common.sh:40-43 |
| `sc_count_unchecked(tracker_file)` | `[ ]` カウント | self_check_common.sh:46-49 |
| `sc_sha16(file)` | sha256 先頭 16 文字 | self_check_common.sh:52-54 |
| `sc_load_config()` | config/self-check.conf をパースして変数に設定 | self_check_common.sh:60-104 |
...
- **処理** (lockdown.sh:1-97):
  ...
  5. `.gate-audit_quarantine/verify-<timestamp>/` を作成し KIT_ROOT を移動
...
### SPEC-E02: ファイル型 Evidence（テスト/Self-check 用）
...
- 根拠: run_tests.sh:27-36, self_check_common.sh:17-26
...
### SPEC-E04: Self-check Evidence

```
logs/evidence/<YYYYMMDD-HHMMSS>_sc_<check_id>.txt
```

- ヘッダ書式: `=== Self-check: <Check Name> ===` + Timestamp (JST/UTC) + Check ID + VERDICT
- 根拠: self_check_common.sh:17-26, self-check.sh:265-278
...
- **スキャン対象**: gate_*.sh、verify_all.sh、verify_gate.sh、evidence.sh、run_tests.sh、self-check.sh
- **検出パターン（13 種）**:
...
### Self-check 設定（config/self-check.conf）

```ini
checks=all                    # 実行チェック (all / 個別列挙 / !除外)
lint_severity=warning         # shellcheck 閾値
regression_baseline_dir=logs/self-check/baseline  # ベースライン保存先
...
doc_pairs=docs/rebuild/...:...,docs/self-check/...:...  # doc 整合ペア
```

根拠: config/self-check.conf:1-22, self_check_common.sh:60-104
...
| 5 | GATE_AUDIT_ROOT 強制 | 全スクリプト必須 (verify_spec.md:12) | generate_handoff.sh のみ（SCRIPT_DIR フォールバックあり） | 低 |
