...
version: 2.0
date: 2026-02-14
status: 正式版（v2.0: 3 層リネーム + context-framework リポジトリ名統一、as-built v2.0 準拠）
...
| 文書 | 役割 | パス |
|------|------|------|
| 要件定義書 | 「なぜ必要か」を定義 | `as_built/as_built_requirements.md`（v2.0） |
| 仕様書 | 「どう動くか」を定義 | `as_built/as_built_spec.md`（v2.0） |
| 実装計画書 | 「どう作ったか」を定義 | `as_built/as_built_implementation_plan.md`（v2.0） |
...
┌──────────────────────────────────────────────────────┐
│  検証キット (KIT_ROOT)                                 │
│  ~/.gate-audit_root/.gate-audit/                  │
│                                                        │
│  ./kit verify    → Gate A〜I を自動検証                 │
│  ./kit test      → Phase 1〜3 のテスト実行              │
│  ./kit self-check → 8 種の CI/QA チェック               │
...
│   └── self_check_task_tracker.md ← CI/QA タスク管理（35/35 ALL_PASS）
│
...
│       └── self_checks/         ← Self-check チェックプラグイン（7 + テンプレート）
│
...
│   └── self-check/baseline/     ← Self-check 回帰検出用ベースライン
│
└── handoff/                     ← 引継ぎ文書
...
| `./kit self-check` | 全チェックが PASS | 1 件以上 FAIL |
...
### 3.3 ./kit self-check [CHECK...]

#### なぜ使うのか

`verify` と `test` が合格しても、**ドキュメント間の整合性** や **命名規約の逸脱** など、Gate 検証ではカバーしきれない品質問題がある。

`self-check`（CI/QA）は、こうした「検証の検証」を行う 8 種のチェックを提供する。人間が見落としがちなメタ品質を機械的にチェックするためのレイヤーである。
...
- **プラグイン方式**: 新しいチェックを追加するには `scripts/lib/self_checks/cq_<key>.sh` を置くだけ（REQ-F08, SPEC-S09）。kit 本体の変更は不要。
...
# config/self-check.conf で除外指定（否定構文）
...
`verify` → `test` → `self-check` → `handoff` を個別に実行するのは面倒である。`./kit all` は **4 ステップを一気通貫で実行する** コマンドである。
...
```bash
# 通常はこれだけ（verify/test/self-check 後に自動的に呼ばれるため、手動で実行する機会は少ない）
./kit handoff
...
1. `GATE_AUDIT_MAIN_REPO` 環境変数（明示指定）
2. `MAIN_REPO` 環境変数（呼び出し元が設定）
3. KIT_ROOT の兄弟ディレクトリを検索
4. `GATE_AUDIT_SEARCH_PATH`（デフォルト: `$HOME/projects`）内を検索
...
6 つのトラッカー（verify, test, as_built, rebuild, post_rebuild, self_check）の進捗を一覧するコマンドである。
...
=== Kit Status ===
  ...
  Self-chk:  35/35 (100%) ALL_PASS

Kit root: /home/masahiro/.gate-audit_root/.gate-audit
...
1. KIT_ROOT が `.gate-audit_quarantine/verify-<timestamp>/` に移動される
...
```bash
# quarantine 内から直接実行
bash ~/.gate-audit_root/.gate-audit_quarantine/verify-<timestamp>/scripts/unlock.sh
...
# または GATE_AUDIT_MAIN_REPO 環境変数を使用
GATE_AUDIT_MAIN_REPO=/home/masahiro/projects/context-framework \
  bash tools/verify_ro_mount_nopasswd_template_v5.sh
```
...
### Q3: 新しい Self-check チェックを追加するにはどうすればよいですか？

`scripts/lib/self_checks/cq_<key>.sh` を作成し、メタデータヘッダ（`@check_key`, `@check_id`, `@check_display`, `@check_order`）と `run_check()` 関数を定義するだけです（PLAN-MAINT02）。
