...
verify_gate_f() {
  ...
  local req1_out="${EVIDENCE_DIR}/req1_summary.txt"
  {
    ...
    echo "--- .repo-id/agent_role_assignment.example.yaml ---"
    if [[ -f "${MAIN_REPO}/.repo-id/agent_role_assignment.example.yaml" ]]; then
      echo "EXISTS"
      record_ref ".repo-id/agent_role_assignment.example.yaml"
      echo ""
      cat "${MAIN_REPO}/.repo-id/agent_role_assignment.example.yaml" 2>/dev/null || true
    else
      echo "NOT FOUND"
    fi
    echo ""
  ...
  local req2_out="${EVIDENCE_DIR}/req2_consistency.txt"
  {
    ...
    for adapter in CLAUDE.md AGENTS.md GEMINI.md; do
      ...
      grep -n -i "initial.setting\|role\|INITIAL_SETTINGS\|.repo-id" "${MAIN_REPO}/${adapter}" 2>/dev/null | head -10 || echo "(none)"
      echo ""
    done

    # Check .gitignore excludes local role files
    echo "--- .gitignore role exclusion ---"
    grep -n -i "role\|repo-id\|agent_role" "${MAIN_REPO}/.gitignore" 2>/dev/null | head -10 || echo "(none)"
    echo ""
  ...
  for adapter in CLAUDE.md AGENTS.md GEMINI.md; do
    if grep -q -i "initial.setting\|INITIAL_SETTINGS\|role\|.repo-id" "${MAIN_REPO}/${adapter}" 2>/dev/null; then
      ((adapter_refs++)) || true
    fi
  done
  ...
  local req3_out="${EVIDENCE_DIR}/req3_functional.txt"
  {
    ...
    ((ct++))
    if grep -q -i "agent_role\|repo-id.*local\|.repo-id/" "${MAIN_REPO}/.gitignore" 2>/dev/null; then
  ...
  if [[ -f "${MAIN_REPO}/WORKFLOW/TOOLING/INITIAL_SETTINGS.md" ]] && [[ -f "${MAIN_REPO}/.repo-id/agent_role_assignment.example.yaml" ]]; then
