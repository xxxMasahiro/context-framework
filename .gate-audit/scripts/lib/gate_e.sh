...
verify_gate_e() {
  ...
  local req1_out="${EVIDENCE_DIR}/req1_summary.txt"
  {
    ...
    for f in _handoff_check/update_runbook.md _handoff_check/task_tracker.md _handoff_check/handoff_prompt.md; do
      echo "${f}:"
    ...
    grep -n -i "言語ポリシー\|Language Policy\|日本語.*正\|canonical\|LANG_POLICY" "${MAIN_REPO}/_handoff_check/update_runbook.md" 2>/dev/null | head -20 || echo "(none)"
    echo ""

    # Check Gate E steps in runbook
    echo "--- Runbook Gate E / STEP-45x references ---"
    grep -n -i "gate.e\|STEP-45\|言語ポリシー" "${MAIN_REPO}/_handoff_check/update_runbook.md" 2>/dev/null | head -20 || echo "(none)"

    record_ref "_handoff_check/update_runbook.md"
  } > "$req1_out"

  # Check if language policy marker exists
  if grep -q "GATE_AUDIT_LANG_POLICY" "${MAIN_REPO}/_handoff_check/update_runbook.md" 2>/dev/null ||
     grep -q "GATE_AUDIT_LANG_POLICY" "${MAIN_REPO}/_handoff_check/task_tracker.md" 2>/dev/null; then
    ...
    if grep -q -i "日本語.*正\|Japanese.*canonical" "${MAIN_REPO}/_handoff_check/update_runbook.md" 2>/dev/null; then
  ...
  local req2_out="${EVIDENCE_DIR}/req2_consistency.txt"
  {
    ...
    grep -n -i "言語\|language\|日本語\|english\|canonical" "${MAIN_REPO}/_handoff_check/task_tracker.md" 2>/dev/null | head -10 || echo "(none)"
    echo ""

    echo "--- Language policy references in handoff ---"
    grep -n -i "言語\|language\|日本語\|canonical" "${MAIN_REPO}/_handoff_check/handoff_prompt.md" 2>/dev/null | head -10 || echo "(none)"
    echo ""
  ...
  if grep -q -i "日本語.*正\|表記ポリシー\|LANG_POLICY\|言語ポリシー" "${MAIN_REPO}/_handoff_check/update_runbook.md" 2>/dev/null; then
  ...
  local req3_out="${EVIDENCE_DIR}/req3_functional.txt"
  {
    ...
    ((ct++))
    if grep -q -i "表記ポリシー\|言語ポリシー\|日本語統一\|LANG_POLICY" "${MAIN_REPO}/_handoff_check/update_runbook.md" 2>/dev/null; then
    ...
    ((ct++))
    if grep -q "GATE_AUDIT_LANG_POLICY\|表記ポリシー\|言語ポリシー" "${MAIN_REPO}/_handoff_check/task_tracker.md" 2>/dev/null; then
    ...
    ((ct++))
    if grep -q -i "STEP-45\|gate.e" "${MAIN_REPO}/_handoff_check/update_runbook.md" 2>/dev/null; then
  ...
  if grep -q -i "表記ポリシー\|日本語統一\|LANG_POLICY" "${MAIN_REPO}/_handoff_check/update_runbook.md" 2>/dev/null; then
