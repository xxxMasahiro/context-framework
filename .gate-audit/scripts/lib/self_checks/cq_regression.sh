...
# @check_order: 99
#
# Uses: SC_BASELINE_DIR (absolute path to baseline directory)
#       KIT_ROOT
#
# Baseline file: ${SC_BASELINE_DIR}/last_run.json
# Format (JSON):
...
run_check() {
  local verdict="PASS"
  local details=""
  local baseline_dir="${SC_BASELINE_DIR_ABS:-${SC_BASELINE_DIR}}"
  ...
  # SC_CURRENT_RESULTS is set by self-check.sh before calling CQ-REG
  # Format: "CQ-TRK:PASS CQ-EVC:PASS CQ-SSOT:FAIL ..."
  if [[ -z "${SC_CURRENT_RESULTS:-}" ]]; then
    details+="DIAG: No current results provided (SC_CURRENT_RESULTS empty)"$'\n'
  ...
  for pair in ${SC_CURRENT_RESULTS}; do
    local ck="${pair%%:*}"
    local vd="${pair##*:}"
