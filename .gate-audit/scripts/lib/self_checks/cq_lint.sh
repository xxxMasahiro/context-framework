...
# Severity and targets are read from config (SC_LINT_SEVERITY, SC_LINT_TARGETS)
#
# @check_key: lint
# @check_id: CQ-LINT
...
run_check() {
  ...
  local severity="${SC_LINT_SEVERITY:-warning}"

  # Resolve lint targets from config
  local -a targets=()
  if [[ ${#SC_LINT_TARGETS[@]} -gt 0 ]]; then
    for pattern in "${SC_LINT_TARGETS[@]}"; do
      [[ -z "$pattern" ]] && continue
