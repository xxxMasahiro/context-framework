...
run_check() {
  ...
  # Get doc pairs from config (SC_DOC_PAIRS loaded by sc_load_config)
  local -a pairs=()
  if [[ ${#SC_DOC_PAIRS[@]} -gt 0 ]]; then
    for p in "${SC_DOC_PAIRS[@]}"; do
      [[ -n "$p" ]] && pairs+=("$p")
    done
  fi

  if [[ ${#pairs[@]} -eq 0 ]]; then
    details+="SKIP: No doc_pairs configured in self-check.conf"$'\n'
    echo "$details"
    return 0
  fi
