...
# ── Discover main repo ──────────────────────────────────────────
# Resolution order:
#   1. GATE_AUDIT_MAIN_REPO env var (explicit override)
#   2. MAIN_REPO env var (if already set by caller)
#   3. Sibling directory: KIT_ROOT/../context-framework
#   4. GATE_AUDIT_SEARCH_PATH entries (colon-separated, default: $HOME/projects)
...
discover_main_repo() {
  # 1. Explicit override
  if [[ -n "${GATE_AUDIT_MAIN_REPO:-}" ]]; then
    if _validate_main_repo "$GATE_AUDIT_MAIN_REPO"; then
      echo "$GATE_AUDIT_MAIN_REPO"
      return 0
    else
      echo "WARN: GATE_AUDIT_MAIN_REPO set but validation failed: ${GATE_AUDIT_MAIN_REPO}" >&2
    fi
  fi
  ...
  # 3. Sibling of kit's parent (common layout: parent/context-framework + parent/.gate-audit)
  local kit="${KIT_ROOT:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)}"
  local sibling
  ...
  # 4. Search under GATE_AUDIT_SEARCH_PATH (default: $HOME/projects)
  local search_path="${GATE_AUDIT_SEARCH_PATH:-${HOME}/projects}"
  local IFS=':'
...
init_evidence() {
  ...
  if [[ -z "$MAIN_REPO" ]]; then
    echo "FATAL: Cannot find main repo. Set GATE_AUDIT_MAIN_REPO or GATE_AUDIT_SEARCH_PATH." >&2
    return 1
  fi
