...
## 1. ディレクトリ仕様
### 1.1 ルート
- **既定（推奨・安全側）**：`$GATE_AUDIT_ROOT/.gate-audit/`（`GATE_AUDIT_ROOT` は必須）
- **例外（明示的オプトイン時のみ）**：`./.gate-audit/`（リポジトリ直下・git管理外）
- `GATE_AUDIT_ROOT` が未設定のまま既定動作を試みた場合、スクリプトは **中断（FAIL）**する
...
- 動作（案）：
  - `.gate-audit/` → `./.gate-audit_quarantine/verify-<timestamp>/` に移動
...
## 7. NOPASSWD mount/umount 検証テンプレート

NOPASSWD で mount/umount のみ許可された環境における bind mount → ro remount → umount の検証は、`tools/verify_ro_mount_nopasswd_template_v5.sh` を使用する。v5 形式（findmnt -T/-M の -o/-no 併記 + PASS 基準の明文出力）に準拠。
