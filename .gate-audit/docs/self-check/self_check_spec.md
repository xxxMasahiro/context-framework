# Self-check 仕様（Self-check Specification）

## 0. 概要

本仕様は、Self-check 要件定義（REQ-CQ01〜CQ08）を実現するためのコマンド設計、チェック構成、入出力、ディレクトリ規約、Runner API を定義する。
...
```
./kit all  →  verify → test → self-check → handoff
```

- self-check は test の後、handoff の前に実行される
- self-check の FAIL は `./kit all` の exit code に反映される（1 を返す）
- self-check が FAIL でも handoff は実行する（最終状態を記録するため）
...
regression_baseline_dir=logs/self-check/baseline

# tracker files to check (comma-separated relative paths)
tracker_files=tasks/verify_task_tracker.md,tasks/test_task_tracker.md,tasks/as_built_task_tracker.md,tasks/rebuild_task_tracker.md,tasks/post_rebuild_task_tracker.md,tasks/self_check_task_tracker.md

# doc pairs to cross-reference (format: req_path:spec_path, comma-separated)
doc_pairs=docs/rebuild/rebuild_requirements.md:docs/rebuild/rebuild_spec.md,docs/self-check/self_check_requirements.md:docs/self-check/self_check_spec.md
```

### 3.2 設定の読み込み順序

1. `config/self-check.conf`（デフォルト）
2. 環境変数 `SC_CONFIG` で指定されたファイル（上書き）
3. コマンドライン引数（最優先）
...
Self-check Summary: 7/7 PASS, 0 FAIL
...
.gate-audit/
├── config/
│   └── self-check.conf                # Self-check 設定
├── docs/self-check/
│   ├── self_check_requirements.md     # Self-check 要件定義
│   ├── self_check_spec.md             # Self-check 仕様（本ファイル）
│   └── self_check_implementation_plan.md  # Self-check 実装計画
├── logs/self-check/
│   └── baseline/                      # 回帰検出用ベースライン
│       └── last_run.json              # 前回実行結果
├── scripts/
│   ├── self-check.sh                  # Self-check 実行器（メインエントリ）
│   └── lib/self_checks/              # 個別チェックスクリプト
...
└── tasks/
    └── self_check_task_tracker.md     # Self-check タスク管理表
```
...
| ファイル | 変更内容 |
|----------|----------|
| `kit` | `self-check` サブコマンドの追加、`all` に self-check ステップ追加 |
| `kit` の `kit_status()` | Self-check トラッカーの表示追加 |
...
各チェックスクリプト (`scripts/lib/self_checks/cq_*.sh`) は以下の契約に従う:
...
| `KIT_ROOT` | 検証キットルート |
| `MAIN_REPO` | 本体repo パス |
| `SC_CHECK_ID` | 実行中のチェック ID（例: `CQ-TRK`） |
| `SC_EVIDENCE_DIR` | Evidence 出力先ディレクトリ |
| `SC_BASELINE_DIR` | 回帰検出用ベースラインディレクトリ |

#### 共通ライブラリ

- `scripts/lib/evidence.sh` を source して `discover_main_repo()` 等を利用可能
- `scripts/lib/self_check_common.sh`（新規）で Self-check 固有のヘルパー関数を提供:
  - `sc_emit_header <check_name>` — Evidence ヘッダ出力
  - `sc_emit_verdict <PASS|FAIL> <reason>` — 判定行出力
  - `sc_count_checked <tracker_file>` — `[x]` カウント
  - `sc_count_unchecked <tracker_file>` — `[ ]` カウント

### 6.2 Runner（self-check.sh）の動作

```
1. config/self-check.conf を読み込む
...
=== Kit Status ===
  ...
  Self-chk:   0/N  (  0%) IN_PROGRESS     ← 新規追加
```
...
Trackers Digest に Self-check トラッカーを追加:

```markdown
### 3.N Self-check Tracker (tasks/self_check_task_tracker.md)
...
- v1.0（2026-02-04 JST）：Self-check 仕様 初版
