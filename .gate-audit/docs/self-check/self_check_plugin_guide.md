# Self-check プラグイン追加手順（Plugin Guide）

## 1. 概要

Self-check はプラグイン方式でチェックを管理する。
`scripts/lib/self_checks/cq_*.sh` に配置されたスクリプトは、起動時に自動検出され実行対象となる。
...
`config/self-check.conf` の `checks=` で実行するチェックを制御できる。
...
`self-check.sh` が各チェック実行前に以下を export する:

| 変数 | 説明 |
|------|------|
| `KIT_ROOT` | 検証キットルートディレクトリ |
| `MAIN_REPO` | 本体リポジトリパス（**read-only**） |
| `SC_CHECK_ID` | 実行中チェックの ID（例: `CQ-TRK`） |
| `SC_EVIDENCE_DIR` | Evidence 出力先ディレクトリ |
| `SC_BASELINE_DIR_ABS` | 回帰検出ベースラインディレクトリ（絶対パス） |

---

## 5. ヘルパー関数（self_check_common.sh）

| 関数 | 説明 |
|------|------|
| `sc_ts_jst` | JST タイムスタンプ |
| `sc_ts_utc` | UTC タイムスタンプ |
| `sc_ts_label` | ファイル名用タイムスタンプ（`YYYYMMDD-HHMMSS`） |
| `sc_emit_header <name> <id>` | Evidence ヘッダ出力 |
| `sc_emit_verdict <PASS/FAIL> <reason>` | 判定行出力 |
| `sc_count_checked <file>` | `[x]` 項目カウント |
| `sc_count_unchecked <file>` | `[ ]` 項目カウント |
| `sc_sha16 <file>` | sha256 先頭 16 文字 |
...
- 新チェック追加後は `./kit self-check` で全チェック PASS を確認すること
