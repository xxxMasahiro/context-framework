# CI/QA タスク管理表（CIQA Task Tracker）

## 目的
検証キットの CI/QA（継続的統合 / 品質保証）レイヤー構築を管理する。
...
- [x] 既存構造の調査（kit CLI / scripts / docs / tasks / evidence）
  - 判定: PASS
  - Evidence: docs/ciqa/ 内の4ドキュメントに調査結果を反映
  - 日時: 2026-02-04 JST

- [x] CI/QA 要件定義（ciqa_requirements.md）作成
  - 判定: PASS
  - Evidence: docs/ciqa/ciqa_requirements.md
  - 日時: 2026-02-04 JST

- [x] CI/QA 仕様（ciqa_spec.md）作成
  - 判定: PASS
  - Evidence: docs/ciqa/ciqa_spec.md
  - 日時: 2026-02-04 JST

- [x] CI/QA 実装計画（ciqa_implementation_plan.md）作成
  - 判定: PASS
  - Evidence: docs/ciqa/ciqa_implementation_plan.md
  - 日時: 2026-02-04 JST

- [x] CI/QA タスク管理表（本ファイル）作成
  - 判定: PASS
  - Evidence: tasks/ciqa_task_tracker.md
  - 日時: 2026-02-04 JST
...
- [x] ciqa_common.sh 作成（ヘッダ出力、verdict 出力、カウント関数）
  - 判定: PASS
  - Evidence: logs/evidence/20260204-035500_ciqa_phase1_implementation.txt
  - 日時: 2026-02-04 04:55 JST

- [x] ciqa_runner.sh 作成（config 読み込み → チェック実行 → Evidence 保存）
  - 判定: PASS
  - Evidence: logs/evidence/20260204-035500_ciqa_phase1_implementation.txt
  - 日時: 2026-02-04 04:55 JST

- [x] config/ciqa.conf 作成（デフォルト設定）
  - 判定: PASS
  - Evidence: logs/evidence/20260204-035500_ciqa_phase1_implementation.txt
  - 日時: 2026-02-04 04:55 JST
...
- [x] kit に ciqa サブコマンド追加
  - 判定: PASS
  - Evidence: logs/evidence/20260204-035500_ciqa_phase1_implementation.txt
  - 日時: 2026-02-04 04:55 JST

- [x] kit_all() に ciqa ステップ追加（verify → test → ciqa → handoff）
  - 判定: PASS
  - Evidence: logs/evidence/20260204-035500_ciqa_phase1_implementation.txt
  - 日時: 2026-02-04 04:55 JST

- [x] kit_status() に CIQA トラッカー表示追加
  - 判定: PASS
  - Evidence: logs/evidence/20260204-035500_ciqa_phase1_implementation.txt
  - 日時: 2026-02-04 04:55 JST

- [x] handoff_builder.sh に CIQA トラッカー追加
  - 判定: PASS
  - Evidence: logs/evidence/20260204-035500_ciqa_phase1_implementation.txt
  - 日時: 2026-02-04 04:55 JST
...
- [x] cq_naming.sh 実装（CQ-NAME: 命名規約チェック）
  ...
  - 日時: 2026-02-04 17:30 JST
- [x] Phase 2 統合テスト（./kit ciqa で 6 チェック実行）
  - 判定: PASS
...
- [x] ベースライン保存ロジック（last_run.json 更新）
  ...
  - 日時: 2026-02-04 18:05 JST
- [x] 受け入れ検証 AC-CQ01（./kit ciqa で全チェック実行 + Evidence 保存）
  - 判定: PASS
...
- [x] 受け入れ検証 AC-CQ06（./kit all に ciqa が統合されている）
  - 判定: PASS
  - Evidence: ./kit all Step 3/4 runs ciqa 7/7 PASS
  - 日時: 2026-02-04 18:04 JST
- [x] 受け入れ検証 AC-CQ07（既存 verify / test / handoff が破壊されていない）
  - 判定: PASS
  - Evidence: ./kit all: verify 9/9, test 3/3, ciqa 7/7, handoff OK
  - 日時: 2026-02-04 18:04 JST
- [x] 受け入れ検証 AC-CQ08（ciqa_task_tracker.md の全 Phase 0 項目が [x]）
  - 判定: PASS
...
- [x] ciqa_runner.sh にプラグイン自動検出ロジック追加
  - 判定: PASS
...
- [x] カスタムチェックテンプレート作成
  - 判定: PASS
  - Evidence: scripts/lib/ciqa_checks/_template.sh, logs/evidence/20260204-145013_ciqa_phase4_plugin.txt
  - 日時: 2026-02-04 23:50 JST
- [x] プラグイン追加手順のドキュメント
  - 判定: PASS
  - Evidence: docs/ciqa/ciqa_plugin_guide.md, logs/evidence/20260204-145013_ciqa_phase4_plugin.txt
  - 日時: 2026-02-04 23:50 JST
...
- 2026-02-04 JST | Phase 0 開始: 既存構造調査 + 4ドキュメント作成 | 判定: PASS | Evidence: docs/ciqa/ 配下 + tasks/ciqa_task_tracker.md
...
- 2026-02-04 14:11 JST | CQ-TRK/CQ-EVC FAIL根絶: verify/test/as_built 3トラッカーにMISSING_DATE/MISSING_VERDICT/MISSING_EVIDENCE補完(49件)＋Evidence参照パス誤り2件修正 → ./kit ciqa 3/3 PASS (exit 0) | Evidence: logs/evidence/20260204-051153_ciqa_summary.txt | sha256(16): b05db5a40c060746
- 2026-02-04 15:02 JST | Phase 2 CQ-DOC実装: cq_docs.sh新規作成＋ciqa_runner.sh登録 → ./kit ciqa 4/4 PASS (exit 0) | Evidence: logs/evidence/20260204-060216_ciqa_summary.txt | sha256(16): e2430be451fd8de6
- 2026-02-04 17:30 JST | Phase 2 完了: cq_lint.sh/cq_naming.sh新規作成＋ciqa_runner.sh拡張＋ciqa.conf追記＋Evidence命名規約違反4件修正 → ./kit ciqa 6/6 PASS (exit 0) | Evidence: logs/evidence/20260204-083000_ciqa_summary.txt | sha256(16): f01ad57b9e0cbad1
- 2026-02-04 18:05 JST | Phase 3 完了: cq_regression.sh新規作成＋ciqa_runner.shにCQ-REG登録＋ベースライン保存ロジック追加＋受け入れ検証AC-CQ01〜08全PASS＋./kit all 一気通貫 ALL PASS → ./kit ciqa 7/7 PASS (exit 0) | Evidence: logs/evidence/20260204-090452_ciqa_phase3_acceptance.txt | sha256(16): e670ce612c6ba8bc
- 2026-02-04 23:50 JST | Phase 4 完了: ciqa_runner.shをプラグイン自動検出方式に改修（glob cq_*.sh＋@metadata解析＋order順ソート）＋config否定記法(!lint等)対応＋_template.sh新規作成＋ciqa_plugin_guide.md新規作成＋全7チェックに@metadataヘッダ追加 → ./kit ciqa 7/7 PASS (exit 0)＋./kit all ALL PASS | Evidence: logs/evidence/20260204-145013_ciqa_phase4_plugin.txt | sha256(16): 8a183590de32dfbd
