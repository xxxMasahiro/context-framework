...
- **検証キットは repo 外生成が既定**（`CFCTX_VERIFY_ROOT` 必須。未設定は FAIL）
- Evidence・トラッカー更新・handoff 生成などの書き込みは **検証キット内のみ**（`$CFCTX_VERIFY_ROOT/.cfctx_verify/` 配下）
...
## 2. 作業パス（前提）
- **KIT_ROOT**: `$CFCTX_VERIFY_ROOT/.cfctx_verify`
