#!/usr/bin/env bash
# ciqa_runner.sh — CI/QA Runner (main entry point)
# Usage: bash scripts/ciqa_runner.sh [CHECK...]
...
#   Check scripts are discovered via glob: scripts/lib/ciqa_checks/cq_*.sh
...
source "${KIT_ROOT}/scripts/lib/ciqa_common.sh"
...
_discover_plugins() {
  local checks_dir="${KIT_ROOT}/scripts/lib/ciqa_checks"
...
if [[ ${#AVAILABLE_CHECKS[@]} -eq 0 ]]; then
  echo "ERROR: No check plugins found in scripts/lib/ciqa_checks/cq_*.sh" >&2
  exit 1
fi
...
resolve_checks() {
  ...
  if [[ -n "${CIQA_CHECKS:-}" && "$CIQA_CHECKS" != "all" ]]; then
    local -a config_items=()
    IFS=',' read -ra config_items <<< "$CIQA_CHECKS"
...
run_single_check() {
  ...
  export CIQA_CHECK_ID="$check_id"
  export CIQA_EVIDENCE_DIR="${KIT_ROOT}/logs/evidence"
  # Resolve CIQA_BASELINE_DIR to absolute (only if relative)
  if [[ "${CIQA_BASELINE_DIR}" == /* ]]; then
    export CIQA_BASELINE_DIR_ABS="${CIQA_BASELINE_DIR}"
  else
    export CIQA_BASELINE_DIR_ABS="${KIT_ROOT}/${CIQA_BASELINE_DIR}"
  fi
  ...
  # Save evidence
  local ts_label
  ts_label="$(ciqa_ts_label)"
  local efile="${KIT_ROOT}/logs/evidence/${ts_label}_ciqa_${check_id}.txt"
  mkdir -p "$(dirname "$efile")"

  {
    ciqa_emit_header "$display" "$check_id"
    ciqa_emit_verdict "$RESULT_VERDICT" "${display} check"
    echo ""
    echo "$details"
  } > "$efile"
...
main() {
  ...
  for check_key in "${check_list[@]}"; do
    ...
    if [[ "$check_key" == "regression" ]]; then
      export CIQA_CURRENT_RESULTS="${summary_lines[*]}"
    fi
  ...
  echo ""
  echo "CIQA Summary: ${total_pass}/${total} PASS, ${total_fail} FAIL"

  # Save summary evidence
  local ts_label
  ts_label="$(ciqa_ts_label)"
  local summary_file="${KIT_ROOT}/logs/evidence/${ts_label}_ciqa_summary.txt"
  {
    ciqa_emit_header "CIQA Summary" "CIQA-ALL"
    if [[ "$total_fail" -eq 0 ]]; then
      ciqa_emit_verdict "PASS" "All ${total} checks passed"
    else
      ciqa_emit_verdict "FAIL" "${total_fail}/${total} checks failed"
    fi
    echo ""
  ...
  local summary_sha
  summary_sha="$(ciqa_sha16 "$summary_file")"
  ...
  local baseline_dir
  if [[ "${CIQA_BASELINE_DIR}" == /* ]]; then
    baseline_dir="${CIQA_BASELINE_DIR}"
  else
    baseline_dir="${KIT_ROOT}/${CIQA_BASELINE_DIR}"
  fi
