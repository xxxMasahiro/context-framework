...
# Verifies: Japanese canonical policy, CFCTX_LANG_POLICY_CANONICAL_V1 marker
...
verify_gate_e() {
  ...
  local req1_out="${EVIDENCE_DIR}/req1_summary.txt"
  {
    ...
    # Check for CFCTX_LANG_POLICY_CANONICAL_V1 marker
    echo "--- CFCTX_LANG_POLICY_CANONICAL_V1 marker search ---"
    for f in _handoff_check/cf_update_runbook.md _handoff_check/cf_task_tracker_v5.md _handoff_check/cf_handoff_prompt.md; do
      echo "${f}:"
      grep -n "CFCTX_LANG_POLICY" "${MAIN_REPO}/${f}" 2>/dev/null || echo "  (not found)"
    done
    echo ""
  ...
  if grep -q "CFCTX_LANG_POLICY" "${MAIN_REPO}/_handoff_check/cf_update_runbook.md" 2>/dev/null ||
     grep -q "CFCTX_LANG_POLICY" "${MAIN_REPO}/_handoff_check/cf_task_tracker_v5.md" 2>/dev/null; then
    write_judgement "PASS" "CFCTX_LANG_POLICY_CANONICAL_V1 marker found in SSOT; language policy established" "req①"
    ((pass++))
  else
  ...
  local req3_out="${EVIDENCE_DIR}/req3_functional.txt"
  {
    ...
    ((ct++))
    if grep -q "CFCTX_LANG_POLICY\|表記ポリシー\|言語ポリシー" "${MAIN_REPO}/_handoff_check/cf_task_tracker_v5.md" 2>/dev/null; then
