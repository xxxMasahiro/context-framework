...
# otherwise uses CFCTX_MAIN_REPO env var.
_hb_init_main_repo() {
  ...
  elif [[ -n "${CFCTX_MAIN_REPO:-}" && -d "${CFCTX_MAIN_REPO}/.git" ]]; then
    MAIN_REPO="$CFCTX_MAIN_REPO"
  fi
  if [[ -z "${MAIN_REPO:-}" ]]; then
    echo "FATAL: Cannot find main repo. Set CFCTX_MAIN_REPO or MAIN_REPO." >&2
    return 1
  fi
  export MAIN_REPO
}
...
emit_trackers_digest() {
  ...
  echo ""
  _hb_digest_tracker \
    "3.6 CIQA Tracker (tasks/ciqa_task_tracker.md)" \
    "$KIT_ROOT/tasks/ciqa_task_tracker.md"
}
