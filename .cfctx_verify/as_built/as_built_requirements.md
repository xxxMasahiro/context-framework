...
| **Tracker** | 検証・テスト・CIQA 等のチェックリスト（`tasks/*.md`）。 | tracker_updater.sh:107-130 |
...
| **CIQA** | CI/QA レイヤー。7 種のチェック（CQ-TRK/EVC/SSOT/DOC/LINT/NAME/REG）。 | ciqa_runner.sh:1-22 |
...
- **実装状態**: 充足。Kit は `/home/masahiro/.cfctx_verify_root/.cfctx_verify/` に存在（repo 外）。
- **根拠**: kit:18-20, evidence.sh:52
- **SSOT との差分**: `verify_requirements.md:34-35` は「CFCTX_VERIFY_ROOT 未設定時に FAIL」と記載。実態は `generate_handoff.sh` のみが CFCTX_VERIFY_ROOT を参照し、他スクリプトは SCRIPT_DIR ベースで自己解決する（→ REQ-D01）。
...
- **実装状態**: 充足。全 Gate スクリプト（gate_a.sh〜gate_i.sh）は `check_file_exists`、`repo_grep`、`repo_grep_capture` 等の参照系関数のみ使用。3 層の検証で担保:
  1. **静的検査（CQ-RO）**: CIQA チェック CQ-RO（cq_readonly.sh）が全 gate/verify/evidence/test/ciqa スクリプトを 13 種の書き込みパターンでスキャンし、write 操作の不在を自動検証。
...
- **実装状態**: 充足。2026-02-06 のセキュリティ総合調査（シェルスクリプト 28 本 + 設定・データファイル）にて全 17 件を精査し、全件 Pass と判定。
  ...
  - Medium: 3 件（sudo mount 変数未検証、CIQA プラグイン無検証 source、awk 変数展開）→ 全件受容（固定値使用・信頼境界内・運用制約上の受容）
...
- **受入条件**: Gate 検証・テスト・CIQA 実行後に、タイムスタンプ付きの Evidence ディレクトリ/ファイルが作成される。
...
- **実装状態**: 充足。`ts_label()` が `date -u +"%Y%m%dT%H%M%SZ"` を返す。CIQA は `ciqa_ts_label()` で `date -u +"%Y%m%d-%H%M%S"` を使用。
- **根拠**: evidence.sh:43, ciqa_common.sh:13
...
- **要件**: `./kit` が handoff / verify / test / ciqa / all / status / lockdown / unlock の各サブコマンドを提供する。
...
### REQ-F04: CI/QA チェック（ciqa）

- **要件**: 8 種のチェック（tracker/evidence/ssot/docs/lint/naming/regression/readonly）を実行できる。
- **受入条件**: `./kit ciqa` で全チェックが PASS/FAIL 判定され、Evidence が保存される。config で有効/無効の切り替えが可能。
- **実装状態**: 充足。`ciqa_runner.sh` がプラグイン自動発見（`cq_*.sh`）を実装。`config/ciqa.conf` で設定。
- **根拠**: ciqa_runner.sh:1-401, config/ciqa.conf:1-22
- **対応 SPEC**: SPEC-S05

### REQ-F05: 一気通貫パイプライン（all）

- **要件**: `./kit all` で verify → test → ciqa → handoff を順次実行できる。
...
### REQ-F08: CIQA プラグイン自動発見

- **要件**: 新チェックの追加は `scripts/lib/ciqa_checks/cq_*.sh` にメタデータヘッダ付きスクリプトを置くだけで完了する。
- **受入条件**: チェックファイルの追加のみで `./kit ciqa` に自動的に含まれる。
- **実装状態**: 充足。`ciqa_runner.sh:48-107` が `@check_key`/`@check_id`/`@check_display`/`@check_order` メタデータをパースして自動登録。
- **根拠**: ciqa_runner.sh:48-107, docs/ciqa/ciqa_plugin_guide.md
- **対応 SPEC**: SPEC-S09
...
### REQ-F10: CIQA プラグインソート（@check_order）

- **要件**: CIQA チェックプラグインの実行順序を `@check_order` メタデータで制御できる。値が小さいほど先に実行される。
- **受入条件**: `@check_order: 10` のプラグインは `@check_order: 50` のプラグインより先に実行される。同一 order のプラグインはキー名のアルファベット順で安定ソートされる。
- **実装状態**: 充足。`ciqa_runner.sh:93-104` が `sort -t: -k1,1n -k2,2` で数値ソート後にアルファベットソートし、安定した実行順序を保証。
- **根拠**: ciqa_runner.sh:93-104
- **対応 SPEC**: SPEC-S09

### REQ-F11: CIQA config 否定構文（checks=!key）

- **要件**: `config/ciqa.conf` の `checks=` で `!` プレフィックスによるチェック除外指定をサポートする。
- **受入条件**: `checks=!lint,!naming` で lint と naming 以外の全チェックが実行される。正と負の混在（例: `tracker,!lint`）はエラーとなる。
- **実装状態**: 充足。`ciqa_runner.sh:146-193` が否定検出・排他モード処理・混在エラー検出を実装。
- **根拠**: ciqa_runner.sh:114-221
- **対応 SPEC**: SPEC-S05
...
- **要件**: CIQA チェック CQ-RO が、全 gate/verify/evidence/test/ciqa スクリプトを対象に、本体 repo への書き込み操作（13 種パターン）が存在しないことを自動検証できる。
- **受入条件**: `./kit ciqa readonly` で CQ-RO チェックが PASS/FAIL 判定される。検出対象は `git push/commit/add/reset/clean/checkout/merge/rebase/stash`、リダイレクト（`>` / `>>`）、`tee/cp/mv/rm/mkdir/touch/chmod/chown/sed -i` のうち `$MAIN_REPO` を対象とするもの。コメント行・文字列リテラル内は除外。
- **実装状態**: 充足。`cq_readonly.sh` が 13 種の write パターンでスキャン。gate_*.sh、verify_all.sh、verify_gate.sh、evidence.sh、run_tests.sh、ciqa_runner.sh を対象。
...
| `./kit ciqa` | 全チェック PASS | 1 件以上 FAIL |
| `./kit all` | 全ステップ PASS | 1 件以上 FAIL |
| `./kit handoff` | 生成成功 | 生成失敗 |
| `./kit status` | 常に 0 | - |

根拠: kit:86, kit:111, kit:131, kit:185, verify_all.sh:144-148, run_tests.sh:471, ciqa_runner.sh:395-398
...
| # | 条件 | 状態 | 根拠 |
|---|------|------|------|
| AC-01 | KIT_ROOT が repo 外に存在 | OK | Kit が `/home/masahiro/.cfctx_verify_root/.cfctx_verify/` に存在 |
...
| AC-04 | tasks/ にトラッカー群（verify/test/as_built/rebuild/post_rebuild/ciqa） | OK | 6 ファイル存在 |
...
| AC-09 | `./kit ciqa` で 8 チェックが PASS/FAIL 判定可能 | OK | ciqa_runner.sh + cq_*.sh 8 ファイル |
...
### REQ-D01: CFCTX_VERIFY_ROOT の扱い不統一

- **SSOT**: verify_requirements.md:34-35 は「CFCTX_VERIFY_ROOT 未設定時にスクリプトは中断（FAIL）する」と記載。
- **実態**: `generate_handoff.sh` のみが CFCTX_VERIFY_ROOT を参照（15-21 行）。ただし SCRIPT_DIR フォールバックあり。他スクリプト（verify_all.sh, verify_gate.sh, kit）は SCRIPT_DIR ベースで自己解決。
- **影響度**: 低（検証は環境変数なしで動作する）
- **提案**: SSOT を実態に合わせて更新するか、全スクリプトに CFCTX_VERIFY_ROOT サポートを追加する。
...
- **実態**: Kit root に README.md なし（`.cfctx_verify/` サブコピー内にのみ存在）。
- **影響度**: 低
