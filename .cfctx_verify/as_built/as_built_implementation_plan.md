...
| KIT_ROOT | `/home/masahiro/.cfctx_verify_root/.cfctx_verify/` | kit:18-20 |
| MAIN_REPO | 本体 repo（自動発見: evidence.sh:18-38） | evidence.sh:18-38 |
| Git | 参照系コマンドのみ使用（status/diff/log/rev-parse） | evidence.sh:250-310 |
| shellcheck | CQ-LINT チェックで使用（未インストール時は SKIP） | ciqa_spec.md:228 |

### PLAN-PRE02: 安全性契約

- 本体 repo への書き込み操作は一切禁止（REQ-S02）。
- 検索コマンドは `|| true` 付与（REQ-S03）。
- Kit 内の書き込み先は `logs/evidence/`、`handoff/`、`tasks/`（トラッカー更新）、`logs/ciqa/baseline/` に限定。
...
| scripts/ 作成 | 完了 | verify_all.sh, verify_gate.sh, generate_handoff.sh, run_tests.sh, ciqa_runner.sh, lib/ 配下 |
...
### PLAN-P4: Phase 4 — CIQA（CI/QA レイヤー）— 完了

| サブフェーズ | タスク | 状態 | 根拠 |
|-------------|--------|------|------|
| Phase 0 | 設計文書作成 (requirements/spec/plan) | 完了 | docs/ciqa/ 配下 4 ファイル |
| Phase 1 | 最小 Runner + コアチェック 3 つ (CQ-TRK/EVC/SSOT) | 完了 | ciqa_runner.sh, cq_tracker.sh, cq_evidence.sh, cq_ssot.sh |
| Phase 2 | kit 統合 + 追加チェック 3 つ (CQ-DOC/LINT/NAME) | 完了 | kit ciqa サブコマンド, cq_docs.sh, cq_lint.sh, cq_naming.sh |
| Phase 3 | 回帰検出 (CQ-REG) | 完了 | cq_regression.sh, logs/ciqa/baseline/ |
| Phase 4 | プラグイン自動発見 | 完了 | ciqa_runner.sh:48-107 (メタデータパース) |
| Phase 4a | プラグインソート (@check_order) | 完了 | ciqa_runner.sh:93-104 (sort -t: -k1,1n -k2,2) |
| Phase 4b | config 否定構文 (checks=!key) | 完了 | ciqa_runner.sh:146-193 |
...
# config で除外指定（否定構文）
# config/ciqa.conf: checks=!lint,!naming  → lint と naming 以外を実行
# 注意: 正と負の混在（例: tracker,!lint）はエラー
./kit ciqa
```

**処理フロー**:
1. config/ciqa.conf を読み込み
...
3. Evidence を確認:
   ```bash
   cat logs/evidence/*ciqa_CQ-TRK*.txt
   ```
...
- `ciqa_runner.sh` の `run_single_check()` が個別 Evidence を保存。
- `main()` がサマリ Evidence を保存。
- 形式: `logs/evidence/<ts>_ciqa_<check_id>.txt` + `<ts>_ciqa_summary.txt`

### PLAN-EV04: ベースライン（回帰検出用）

- `ciqa_runner.sh` の最後に `logs/ciqa/baseline/last_run.json` を更新。
- 次回 `CQ-REG` チェック時に前回結果と比較。
- 根拠: ciqa_runner.sh:362-393
...
### PLAN-MAINT02: 新 CIQA チェック追加

1. `scripts/lib/ciqa_checks/cq_<key>.sh` を作成
...
4. **他ファイルの変更は不要**（ciqa_runner.sh が自動発見）
...
6. 根拠: ciqa_runner.sh:48-107, docs/ciqa/ciqa_plugin_guide.md
...
| 2 | CFCTX_VERIFY_ROOT 統一 | 低 | generate_handoff.sh 以外は SCRIPT_DIR 自己解決 | verify_requirements.md:35 |
...
| 検証が "ドキュメント確認" だけで終わる | verify_implementation_plan.md:91 | verify_all.sh による自動検証 + checksums 検算 + CIQA 自動チェック | 充足 |
...
**調査対象**:
- kit（メイン CLI）、scripts/*.sh、scripts/lib/*.sh、scripts/lib/ciqa_checks/cq_*.sh、tools/*.sh
- config/ciqa.conf、.gitignore、Git 追跡ファイル（.bak, .pyc）、ファイル権限
