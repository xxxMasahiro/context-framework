...
- `CFCTX_VERIFY_ROOT` を確認（未設定なら中断 / 例外は明示オプトイン）
- `$CFCTX_VERIFY_ROOT/.cfctx_verify/` を生成（既定・推奨）
...
**完了条件**
- `CFCTX_VERIFY_ROOT` が設定され、検証キットが **リポジトリ外**に生成されている（既定）
...
## 4. 実装タスクリスト（キット生成側）
- [ ] `CFCTX_VERIFY_ROOT` を確認し、既定では **リポジトリ外**にキットを生成する
- [ ] `.cfctx_verify/` を作成（例外は明示オプトイン）
