...
## 3. 安全性要件（MUST）
### 3.1 生成場所の安全（MUST）
- 既定の検証キットルートは **`$CFCTX_VERIFY_ROOT/.cfctx_verify/`** とする（リポジトリ外生成を推奨）。
- `CFCTX_VERIFY_ROOT` 未設定時に既定動作を試みた場合、スクリプトは **中断（FAIL）**する。
...
- 検証キットが `$CFCTX_VERIFY_ROOT/.cfctx_verify/` に存在し、次が揃っている：
  - `SSOT/` に3ファイル（参照コピー）
