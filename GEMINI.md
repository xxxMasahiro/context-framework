...
## 目的
- Google Antigravity / Gemini 系に **直接渡す運用アダプタ**として使う
- SSOT（唯一の正）は cf-context-framework 側に固定する

## 役割（固定しない）
- このmdは Gemini 向けの入口（読み込み必須）であり、役割は固定しない
...
## やること
- SSOTを参照し、指示は1手ずつ出す
- Skill優先実行（無ければ作成提案）を守る
- 変更点は追加/削除/修正を明示する
- 実行コマンドの意味（復習用）を添える
- 参照リンク中心で記述し、本文の重複を避ける

## やらないこと
- SSOTの再定義や本文の複製
- main への直接コミット（PR経由で反映すること）
