...
## app/ 変更時の Gate D 免除条件（REQ-CF-I11）

変更が `app/**` のみの場合、Gate D 監査は省略可能です。

- **判定方法**: `git diff --name-only` で変更ファイル一覧を取得し、全ファイルが `app/` 配下であるかを判定する。
- **CI/CQ 証跡**: Gate D が省略されても、ciqa パイプラインの実行結果（evidence アーティファクト）は証跡として残ります。
- **L1/L2 を含む変更**: 既存どおり Gate D 監査が必要です。
